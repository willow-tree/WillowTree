...
def test_pattern():
    import itertools
    coloriter = itertools.cycle(diagnostic_colors)
    vines = [[coloriter.next()] * 35 for i in range(40)]
    
    while True:
        for channel, vinepixels in enumerate(vines): 
            client.put_pixels(vinepixels, channel=0)
