...
def cycle_in_cosine(branch_vine,t,period):
	return math.cos((2 * math.pi /period) * (t + 1.0 * (branch_vine + 1) / constants.num_vines_per_branch))
...
def cycle_up_cosine(vine_pixel,t,period):
	return math.cos((2 * math.pi / period) * (t + 1.0 * (vine_pixel + 1) / constants.num_lights_per_vine))
